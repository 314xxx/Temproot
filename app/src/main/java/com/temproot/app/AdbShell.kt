...
object AdbShell {
    ...
    /**
     * 网络超时（毫秒）。
     * 关键：Kadb.create 的默认 connectTimeout/socketTimeout 均为 0（无限等待），
     * 曾导致点击连接后无限挂起且互斥锁被占、界面毫无反应——必须显式指定。
     */
    ...
    /** 初始化（应用启动时调用一次）。证书加载延迟到首次配对/连接时在 IO 线程执行，避免主线程 I/O。 */
    fun init(context: Context) {
        if (appContext != null) return
        appContext = context.applicationContext
    }

    /** 恢复持久化证书（IO 线程，一次性；未配对时静默跳过） */
    private suspend fun ensureCertLoaded() {
        if (certLoaded) return
        certLoaded = true
        val ctx = appContext ?: return
        withContext(Dispatchers.IO) {
            val certFile = File(ctx.filesDir, "kadb_cert.pem")
            val keyFile = File(ctx.filesDir, "kadb_key.pem")
            if (certFile.exists() && keyFile.exists()) {
                runCatching { KadbCert.set(certFile.readBytes(), keyFile.readBytes()) }
                    .onFailure { Log.w(TAG, "证书加载失败: ${it.message}") }
            }
        }
    }
     ...
     */
    suspend fun pair(pairingCode: String, discoveredPort: Int? = null): Result<Unit> {
        ensureCertLoaded()
        return withContext(Dispatchers.IO) {
            runCatching {
                ...
                val port = discoveredPort
                    ...
                    ?: throw Exception("未发现配对服务\n请先在系统设置打开「使用配对码配对设备」")

                // 配对为阻塞式 TLS 握手：整体限时，防止无限挂起
                try {
                    withTimeout(PAIR_TIMEOUT_MS) { Kadb.pair(HOST, port, pairingCode.trim()) }
