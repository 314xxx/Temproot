...
2. 回到 TempRoot，点击「配对」（弹出系统样式配对对话框，内可一键跳转系统无线调试设置页）
3. 在手机上点击「使用配对码配对设备」——应用会**自动检测配对服务**（mDNS 发现，无需输入端口）
4. 输入 6 位配对码，三种方式任选：
   - **点击通知** → 直接弹出系统样式配对对话框输入（无需回到应用主界面）
   - **通知栏内直接输入**（RemoteInput，像回复短信一样）
   - 在应用内配对对话框输入
5. 配对成功后自动发现连接端口并连接，结果通过通知反馈

> 配对端口与连接端口均由 mDNS 自动发现，全程无需手动输入任何端口。
> 配对证书已持久化保存，之后无需再次配对。
> Android 13+ 首次启动会请求通知权限（用于通知输入配对码）。
...
│   │   │   ├── PairingNotifier.kt # 配对通知（RemoteInput 通知栏输入配对码）
│   │   │   ├── PairingReplyReceiver.kt # 通知回复处理（后台配对+连接）
│   │   │   ├── CrashHandler.kt    # 崩溃日志捕获（闪退排查）
